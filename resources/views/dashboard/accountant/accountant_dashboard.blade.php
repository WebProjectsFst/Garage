...
@section("imports")
    <script type="text/javascript">
        function dailyRevenue() {
            $.post("/factureController/daily", {_token: "{{ csrf_token() }}"}, function (data, status) {
                if(status == "success") {
                    var somme=0;
        ...
        function monthlyRevenue() {
            $.post("/factureController/monthly", {_token: "{{ csrf_token() }}"}, function (data, status) {
                if(status == "success") {
                    var somme=0;
                    data.forEach(function (facture) {
                        somme+=facture.montant;
        ...
        function annualRevenue() {
            $.post("/factureController/annual", {_token: "{{ csrf_token() }}"}, function (data, status) {
                if(status == "success") {
                    var somme=0;
                    data.forEach(function (facture) {
                        somme+=facture.montant;
        ...
        function pay(element) {
            ...
            $.post("/factureController/payBills", {_token: "{{ csrf_token() }}", id: prestrationId}, function(data, status) {}, "json");
            page3_init();
        }
        ...
        function genereateFacture(element) {
            ...
            $.post("/getFichePrestationDetailByID", {_token: "{{ csrf_token() }}", id: prestrationId}, function(data, status) {
                if(status == "success") {
