...
@section("imports")
    <script type="text/javascript">
        ...
        function page1_init() {
            ...
            //Prestation
            $.post("/prestations", {_token: "{{ csrf_token() }}"}, function(data, status) {
            ...
            //Workers[
            $.post("/workers", {_token: "{{ csrf_token() }}"}, function(data, status) {
                ...
                if(status == "success") {
                    ...
                    data.forEach(function (worker){
                        $.post("/fichePrestationByNSS", {_token: "{{ csrf_token() }}", nss_employee: worker.NSS}, function (data2, status2) {
                            if(status2 == "success"){
                                if(data2!=""){
        ...
        function cinHelper(element) {
            ...
            if(searchedCin){
                $.post("/clients", {_token: "{{ csrf_token() }}", existing_cin: searchedCin}, function(data,status){
                    if(status == "success"){
                        var html="";
                        clients=data;
